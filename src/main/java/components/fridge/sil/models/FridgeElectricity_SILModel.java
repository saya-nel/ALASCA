...
@ModelExternalEvents(imported = {
		SetEco.class, SetNormal.class,
		SetRequestedTemperature.class,
		Activate.class,
		Passivate.class})
public class FridgeElectricity_SILModel extends AtomicHIOA {


	public static final String 		URI = FridgeElectricity_SILModel.class.getSimpleName();
	 ...
	 * @throws Exception
	 */
	public FridgeElectricity_SILModel(String uri, TimeUnit simulatedTimeUnit, SimulatorI simulationEngine)
			throws Exception {
	...
	/**
	 * update the intensity of electric concsumption of the fridge given its
	 * current mode.
	 *
	 * <p><strong>Contract</strong></p>
	 *
	...
	protected void updateIntensity()
	{
		if(this.isSuspended){
			...
			if(this.currentMode==FridgeMode.NORMAL){
				this.currentIntensity.v = this.NORMAL_MODE_CONSUMPTION/TENSION;
			} else{
				this.currentIntensity.v = this.ECO_MODE_CONSUMPTION/TENSION;
			}
		}
